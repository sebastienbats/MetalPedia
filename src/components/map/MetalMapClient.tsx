...
const Globe = dynamic(() => import('react-globe.gl'), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-[600px] w-full bg-metal-black/50 rounded-lg border border-metal-gray">
      <Loader text="Initialisation du moteur 3D..." />
    </div>
  ),
});
      ...
      try {
        ...
        const mappedCountries: CountryData[] = Object.entries(countryCounts)
          .map(([name, count]) => {
            ...
            return { name, lat: info.lat, lng: info.lng, bandCount: count, flag: info.flag };
          })
          .filter((c) => c.lat !== 0)
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-[600px] bg-metal-black/50 rounded-lg border border-metal-gray">
        <Loader text="Analyse des données et génération du globe..." />
      </div>
    );
  }
